...
namespace MissionPlanner
{
    public static class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            log.Info("**************** Check and Create logs dir ***************");
            string subPath = "//logs"; // your code goes here

            bool isExists = System.IO.Directory.Exists(Directory.GetCurrentDirectory() + subPath);
            // Console.WriteLine(Directory.GetCurrentDirectory() + subPath);
            if (!isExists)
            {
                System.IO.Directory.CreateDirectory(Directory.GetCurrentDirectory() + subPath);
            }
                     log.Info("**************** Check permission on installation dir ***************");
           
            
            
            if (!Settings.Default.AccessOnFolder)
            {
                ...
                if (dirInfo != null)
                {
                    ...
                    Settings.Default.AccessOnFolder = true;
                    log.Info("**************** Permission is OK ***************");
                }
            }
            else {
                   
                    log.Info("**************** Permission not need to permorf update ***************");
            
            }

            if (Properties.Settings.Default.firstLaunch)
            {
                Properties.Settings.Default.license = "00000";
                Properties.Settings.Default.owner = "Versione Prova";
                Properties.Settings.Default.firstLaunch=false;
                log.Info("**************** license installation ***************");
            }
